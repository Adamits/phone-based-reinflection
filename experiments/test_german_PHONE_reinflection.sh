...
python "$ROOT"/OpenNMT-py/translate.py -model "$MODEL" -src "$ROOT"/phone-based-reinflection/data/onmt-phone-inputs/"$LANG"-test-src.txt -tgt "$ROOT"/phone-based-reinflection/data/onmt-phone-inputs/"$LANG"-test-tgt.txt  -output "$ROOT"/phone-based-reinflection/preds/"$MODEL_FN"-pred.txt -replace_unk -gpuid 0
