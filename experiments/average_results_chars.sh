...
for l in bengali catalan dutch english french german hindi hungarian italian kurmanji persian polish portuguese russian sorani spanish swedish ukrainian; do
  ...
  python "$ROOT"/OpenNMT-py/translate.py -model "$MODEL" -src "$ROOT"/phone-based-reinflection/average_char_inputs/"$l"-uncovered-test-src.txt -tgt "$ROOT"/phone-based-reinflection/average_char_inputs/"$l"-uncovered-test-tgt.txt  -output "$ROOT"/phone-based-reinflection/average_char_preds/"$l"-pred.txt -replace_unk
