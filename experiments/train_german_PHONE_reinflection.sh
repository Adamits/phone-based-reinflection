...
python "$ROOT"/phone-based-reinflection/scripts/conll2onmt-epitran_PHONES.py "$LANG" "$ROOT"/"$LANG"-dev

# Try all settings
for s in low medium high;
do
  # Format the train data, and get the g2p
  python "$ROOT"/phone-based-reinflection/scripts/conll2onmt-epitran_PHONES.py "$LANG" "$ROOT"/"$LANG"-train-"$s"
